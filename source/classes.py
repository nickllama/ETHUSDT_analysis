...
from source.func import check_eth_price, print_suc_del, find_regression_coefficient
...
if __tablename__ is None:
    ...
    print(f"TABLE_NAME is set to: {__tablename__}")
...
class FuturesTrade(Base):
    ...
    """
    __tablename__ = __tablename__  # Используем значение переменной окружения
...
class FuturesProcessor:
    ...
    def create_table(self):
        ...
        """
        if not self.engine.dialect.has_table(self.engine.connect(), __tablename__):
    ...
    async def read_data_to_dataframe(self):
        ...
        """
        trades = self.session.query(FuturesTrade).filter_by(symbol=self.symbol.upper()).all()
    ...
    async def delete_old_trades(self):
        ...
        try:
            ...
            old_trades = self.session.query(FuturesTrade).filter(FuturesTrade.timestamp < sixty_minutes_ago).all()
            if old_trades:
                await print_suc_del()
        ...
        async with websockets.connect(
            ...
            while True:
                try:
                    ...
                    handle_trade_task = asyncio.create_task(self.handle_trade(response))
                    delete_old_trades_task = asyncio.create_task(self.delete_old_trades())
